  ...
  return (
    <div className="bg-surface min-h-screen text-on-surface font-[var(--font-inter)] dark:bg-slate-950 flex flex-col">
        ...
        <div className="mt-20 flex flex-col h-full">
          ...
          <nav className="space-y-1 flex-1">
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ease-in-out ${isActive("/dashboard/inventory")
                ? "bg-white dark:bg-slate-800 text-blue-700 dark:text-blue-300 shadow-sm"
                : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/80"
                }`}
            >
              <Package className={`w-5 h-5 ${isActive("/dashboard/inventory") ? "fill-blue-700/20" : ""}`} />
      ...
      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-6 pt-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-[0_-12px_40px_rgba(0,0,0,0.05)] lg:hidden z-50 rounded-t-[2rem]">
        ...
        <Link
          href="/dashboard/inventory"
          className={`flex flex-col items-center justify-center px-3 py-2 tap-highlight-transparent scale-90 active:scale-100 transition-transform ${isActive("/dashboard/inventory") ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-2xl" : "text-slate-400 dark:text-slate-500"
            }`}
        >
          <Package className={`w-6 h-6 ${isActive("/dashboard/inventory") ? "fill-blue-700/20" : ""}`} />
